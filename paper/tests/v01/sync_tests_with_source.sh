...
# SRC_DIR="$(realpath "${THIS_DIR}/../contents/mngs")"
...
usage() {
    ...
    echo "Example:"
    echo "  $0 --move"
    echo "  $0 --source /path/to/contents --tests /path/to/tests"
    exit 1
}
...
get_source_code_block() {
    ...
    echo ""
    echo "# --------------------------------------------------------------------------------"
    echo "# Start of Source Code from: $contents_file"
    echo "# --------------------------------------------------------------------------------"
    sed 's/^/# /' "$contents_file"
    echo ""
    echo "# --------------------------------------------------------------------------------"
    echo "# End of Source Code from: $contents_file"
    echo "# --------------------------------------------------------------------------------"
}
...
EOL
        ...
        get_source_code_block "$contents_file" >> "$test_file"
    else
        ...
        # Add source code block
        get_source_code_block "$contents_file" >> "$temp_file"
...
move_stale_test_files_to_old() {
    ...
    find "$TESTS_DIR" -name "test_*.py" -not -path "*.old*" | while read -r test_path; do
        ...
        contents_filename="${test_filename#test_}"
        contents_rel_dir="$test_rel_dir"
        contents_rel_path="$contents_rel_dir/$contents_filename"
        contents_path="$SRC_DIR/$contents_rel_path"

        if [ ! -f "$contents_path" ] && [ ! -f "$test_path" ]; then
...
main() {
    ...
    find_files "$SRC_DIR" f "*.py" | while read -r contents_file; do
        # derive relative path and parts
        rel="${contents_file#$SRC_DIR/}"
        rel_dir=$(dirname "$rel")
        contents_base=$(basename "$rel")
        ...
        test_file="$tests_dir/test_$contents_base"

        # Process each file
        update_test_file "$test_file" "$contents_file"
    done
